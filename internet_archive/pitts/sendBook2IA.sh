...
--header 'x-archive-meta01-collection:americanmethodism' \
--header 'x-archive-meta-sponsor:Emory University, Pitts Theology Library' \
--header 'x-archive-meta-contributor:Emory University, Pitts Theology Library' \
--header 'x-archive-meta-ppi:300' \
--header "authorization: LOW fpJYfLnxbLmCEHqU:JQbq6NgBaPrSh98Y" \
--upload-file /mnt/lsdi/${1}-${2}/${1}/${1}_MRC.xml \
...
echo zip -r /temp/${1}_${2}_images.zip /mnt/lsdi/${1}-${2}/${1}/Output/*.tif

# need to echo command to console
zip -r /temp/${1}_${2}_images.zip /mnt/lsdi/${1}-${2}/${1}/Output/*.tif
